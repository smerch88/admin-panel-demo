...
export const TeammatesStats: React.FC<TeammatesStatsProps> = ({ locale }) => {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-sm font-medium">Мова</CardTitle>
          <Users className="h-4 w-4 text-muted-foreground" />
        </CardHeader>
        <CardContent>
          <div className="text-2xl font-bold">{locale.toUpperCase()}</div>
          <p className="text-xs text-muted-foreground">Поточна мова</p>
        </CardContent>
      </Card>
    </div>
  );
